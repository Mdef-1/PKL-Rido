...
<div class="container py-5">
    ...
    <div class="row mb-4">
        <div class="col-12">
            <a href="{{route('orders.my')}}" class="btn btn-outline-secondary">&laquo; Kembali ke Daftar Pesanan</a>
        </div>
    </div>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    <div class="col-md-6">
                        ...
                        <p>{{$order->status == 'completed' ? 'Selesai' : 'Menunggu Pesanan'}}</p>
                    </div>
                    </div>
                </div>
            </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if ($item->product->gambar)
                                                <img src="{{asset('storage/'. $item->product->gambar)}}" alt="" width="80" class="img-thumbnail">
                                            @else
                                        ...
                                        <td>
                                            @if ($order->status == 'pending')
                                                <form action="{{route('order.update.quantity')}}" method="POST" class="d-flex align-items-center">
                                                    ...
                                                    <input type="number" name="quantity" value="{{$item->quantity}}" min="1" max="{{$item->product->stok}}" class="form-control form-control-sm" style="width: 60px;">
                                                    <button type="submit" class="btn btn-sm btn-outline-primary ms-2">Perbarui</button>
                                                </form>
                                                <small class="text-muted d-block mt-1">Tersedia: {{$item->product->stok}}</small>
                                            @else
                                                {{$item->quantity}}
                                            @endif
                                        </td>
